...
logfile = f'{dir_path}/logs/obsidian_api_{datetime.now().strftime("%H_%M_%d_%m_%Y")}.log'
...
class ObsidianFiles():
    def __init__(
        ...
        self.cert = (public_cert, public_key) if public_cert and public_key else None # certifi.where()
    ...
    def _send_request(
        ...
        _request = Request(
            ...
            data=data,
            ) if data else Request(
                method,
                f"{self.api_url}{cmd}",
                headers=self.headers,
                )
        prepped = s.prepare_request(_request)
        resp = s.send(prepped, cert=self.cert) if self.cert else s.send(prepped, verify=False) 
        return resp
    ...
    def _append_content_to_target_file(
        self, 
        target_filename: str, 
        content: str
        ):
    ...
    def _search_with_query(self, request_body: str or dict) -> List[dict[str, any]]:
        ...
        try:
            self.headers['accept'] = 'application/json'
            self.headers['Content-Type'] = 'application/vnd.olrapi.dataview.dql+txt' if isinstance(request_body, str) else  'application/vnd.olrapi.jsonlogic+json'
